...
def validate(model, loader, find_thres=False):
    ...
    with torch.no_grad():
        ...
        for img, label in loader:
            in_tens = img.cuda()
            # (mu, std), logit = model(in_tens)
            (mu, std), logit = model(in_tens)
            y_pred.extend(logit.sigmoid().flatten().tolist())
    ...
    if not find_thres:
        return ap, r_acc0, f_acc0, acc0, auc_score
    ...
    return ap, r_acc0, f_acc0, acc0, r_acc1, f_acc1, acc1, best_thres, auc_score
...
class RealFakeDataset(Dataset):
    def __init__(self,  real_path, 
        ...
        assert data_mode in ["wang2020", "ours", "manually"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_mode', type=str, default='', help='wang2020 or ours')
    parser.add_argument('--max_sample', type=int, default=100, help='only check this number of images for both fake/real')
    ...
    model_= state_dict['model']
    # print(model_dice.keys())
    ...
    for dataset_path in (dataset_paths):
        ...
        ap, r_acc0, f_acc0, acc0, r_acc1, f_acc1, acc1, best_thres, auc_score = validate(model, loader, find_thres=True)

# ###############
        with open( os.path.join(opt.result_folder,'ap.txt'), 'a') as f:
            f.write(dataset_path['key']+': ' + str(round(ap*100, 2))+'  '+str(round(auc_score, 2))+'\n' )
...
# ###############
#         with open( os.path.join(opt.result_folder,'ap.txt'), 'a') as f:
#             f.write('glide'+': ' + str(round(ap*100, 2))+'  '+str(round(auc_score, 2))+'\n' )
#
#         with open( os.path.join(opt.result_folder,'acc0.txt'), 'a') as f:
#             f.write('glide'+': ' + str(round(r_acc0*100, 2))+'  '+str(round(f_acc0*100, 2))+'  '+str(round(acc0*100, 2))+'\n' )
